...
namespace Proyecto_CAI_Grupo_4
{
    public partial class GenerarReserva : Form
    {
        ...
        {
            var dni = dniClientetxb.Text.Trim();


            var filteredReservas = reservas
                .Where(x => (string.IsNullOrEmpty(dni) || x.Codigo == dni));

            var validacion = ValidarBuscarReservas(filteredReservas, dni);
        ...
        private bool ValidarBuscarReservas(IEnumerable<Reserva> list, string nroDeDoc)
        {
            if (list.Any())
            {
                return true;
        ...
        private void OpenCasoDeUso5Form(object? obj)
        {
